...
#if CONFIG_LSM6DS3_GPIO_ENABLE
#include <hal/nrf_gpio.h>
#endif

#include "lsm6ds3.h"

LOG_MODULE_REGISTER(LSM6DS3, 4);
...
#if CONFIG_LSM6DS3_GPIO_ENABLE
#define GPIO_ENABLE_PIN NRF_GPIO_PIN_MAP(CONFIG_LSM6DS3_ENABLE_GPIO_PORT, CONFIG_LSM6DS3_ENABLE_GPIO_PIN)
static int lsm6ds3_enable_gpio_init(const struct device *dev)
{
	nrf_gpio_pin_set(GPIO_ENABLE_PIN);
	nrf_gpio_cfg(GPIO_ENABLE_PIN, NRF_GPIO_PIN_DIR_OUTPUT, NRF_GPIO_PIN_INPUT_DISCONNECT, NRF_GPIO_PIN_NOPULL, NRF_GPIO_PIN_D0H1, NRF_GPIO_PIN_NOSENSE);
	k_busy_wait(USEC_PER_MSEC * 20);
	return 0;
}
#endif
...
#ifdef CONFIG_LSM6DS3_GPIO_ENABLE
	ret = lsm6ds3_enable_gpio_init(dev);
	if(ret){
		LOG_ERR("Enable GPIO init fail");
		return ret;
	}
	LOG_DBG("Enable GPIO configured");
